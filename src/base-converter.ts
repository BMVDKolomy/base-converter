...
export class BaseConverter {
  ...
  private validateNumberInBaseN(
    ...
    const splittedNumberByDelimeter = numberToConvert.split(this.delimiter);
    if (
      splittedNumberByDelimeter.length < 1 ||
      splittedNumberByDelimeter.length > 2
    ) {
    ...
    for (const digit of digits) {
      const digitAsNumber = this.geDigitAsNumber(digit);
  ...
  convertFromBaseNToDecimal(numberToConvert: string, baseFrom: number): number {
    // 1101 (binary) -> 2^3 * 1 + 2^2 * 1 + 2^1 * 0 = 8 + 4 + 0 + 1 = 13
    ...
    for (const digit of digitsArray) {
      result += Math.pow(baseFrom, position) * this.geDigitAsNumber(digit);
      position--;
    }
    return result;
  }
  private geDigitAsNumber(digit: string) {
  ...
  convertDecimalPart(decimalPart: number, baseTo: number, precision: number) {
    let result = "";
  ...
  private validateDecimalNumber(
    ...
    if (typeof numberToConvert !== "number") {
      throw Error("Number to convert needs to be a number");
    }
  }
   ...
   * @param __namedParameters A objectwith the options for the convert method to work

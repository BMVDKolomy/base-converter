...
describe("convertFromBaseNToDecimal()", () => {
  ...
  describe("When given a number in base N with an integer and decimal part", () => {
    test("Should convert it to a decimal number", () => {
      ...
      expect(converter.convertFromBaseNToDecimal("1234.12", 8)).toBeCloseTo(
        668.15625
      );
    });
  });
  ...
  describe("when given an invalid number in base N", () => {
    test("should throw an error indicating that the number is no a string", () => {
      expect(() =>
    ...
    test("should th an error indicating that the baseFrom must be a number", () => {
      expect(() =>
        converter.convertFromBaseNToDecimal("123", "5" as unknown as number)
      ).toThrowError("Base needs to be an integer");
    });
  });
...
describe("convertFromDecimalToBaseN()", () => {
  describe("When given a decimal number", () => {
    test("should converted it to a base N number", () => {
  ...
  describe("When given invalid parameters to the method", () => {
    ...
    test("should throw an error indicating that the number to convert needs to be a number", () => {
      expect(() =>
        converter.convertFromDecimalToBaseN("test" as unknown as number, 12)
...
describe("convert()", () => {
  describe("When given a number in base N and a number in base M", () => {
    test("should convert the number between these bases", () => {
      ...
      expect(converter.convert("10.11", { fromBase: 5, toBase: 2, precision: 3 })).toEqual(
        "101.001"
      );
    });
  });
